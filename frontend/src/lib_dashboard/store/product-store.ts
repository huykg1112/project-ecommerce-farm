...
import type { Product, Category, Disease, ActiveIngredient } from "@/types/entities"
...
export const useProductStore = create<ProductStore>((set, get) => ({
  ...
  setPagination: (page, totalPages, totalItems) => set({ currentPage: page, totalPages, totalItems }),
  ...
  selectAllProducts: () => set((state) => ({ selectedProducts: state.products.map((p) => p.product_id) })),
  clearSelection: () => set({ selectedProducts: [] }),

  // Modal actions
  openAddModal: () => set({ isAddModalOpen: true }),
  openEditModal: (product) => set({ isEditModalOpen: true, editingProduct: product }),
  openLockModal: (product) => set({ isLockModalOpen: true, lockingProduct: product }),
  closeModals: () =>
    set({
      isAddModalOpen: false,
      isEditModalOpen: false,
