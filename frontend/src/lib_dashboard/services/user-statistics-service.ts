...
export class UserStatisticsService {
    ...
    try {
      ...
      if (filters.role !== "all") {
        filteredUsers = filteredUsers.filter(
          (user) => user.role.role_name === filters.role
        );
      }
      ...
      const registrationTrends = this.generateRegistrationTrends(
        allUsers,
        filters
      );
  ...
  private static generateRegistrationTrends(
    users: User[],
    filters: UserStatisticsFilters
  ): RegistrationData[] {
    ...
    if (filters.timeRange === "month") {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const customers = monthUsers.filter(
          (user) => user.role.role_name === "CUSTOMER"
        ).length;
        const distributors = monthUsers.filter(
          (user) => user.role.role_name === "DISTRIBUTOR"
        ).length;
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        const customers = yearUsers.filter(
          (user) => user.role.role_name === "CUSTOMER"
        ).length;
        const distributors = yearUsers.filter(
          (user) => user.role.role_name === "DISTRIBUTOR"
        ).length;
      ...
      while (current <= end) {
        const monthName = current.toLocaleDateString("vi-VN", {
          month: "short",
          year: "numeric",
        });
        ...
        const monthUsers = users.filter((user) => {
          const userDate = new Date(user.created_at);
          return (
            userDate.getMonth() === current.getMonth() &&
            userDate.getFullYear() === current.getFullYear()
          );
        });
