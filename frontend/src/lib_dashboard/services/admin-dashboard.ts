import type { TimeRange } from "@/lib_dashboard/store/dashboard";
...
export const generateMockDashboardData = (timeRange: TimeRange) => {
  ...
  const revenueData = Array.from(
    { length: timeRange === "day" ? 24 : timeRange === "week" ? 7 : 12 },
    (_, i) => {
      const period =
        timeRange === "day"
          ? `${i}:00`
  ...
  return {
    ...
    totalRevenue:
      Math.floor(Math.random() * 100000000 * baseMultiplier) + 50000000,
    revenueData,
    userDistribution,
    recentActivities,
    quickStats: {
