...
let categories = [...mockCategoriesData];
...
export const categoryAPI = {
  ...
  async updateCategory(categoryData: UpdateCategoryRequest): Promise<Category> {
    ...
    const categoryIndex = categories.findIndex(
      (category) => category.category_id === categoryData.category_id
    );
    if (categoryIndex === -1) {
    ...
    const updatedCategory: Category = {
      ...currentCategory,
      category_name:
        categoryData.category_name ?? currentCategory.category_name,
  ...
  async batchToggleStatus(
    ...
    categoryIds.forEach((categoryId) => {
      ...
      if (categoryIndex !== -1) {
        categories[categoryIndex].is_active = status;
        categories[categoryIndex].updated_at = new Date();
  ...
  async batchDeleteCategories(categoryIds: string[]): Promise<boolean> {
    await delay(600);

    categories = categories.filter(
      (category) => !categoryIds.includes(category.category_id)
    );
...
export const warehouseAPI = {
  async getWarehouses(
  ...
  ): Promise<PaginatedResponse<Invenstory>> {
    await delay(300);
    ...
    if (distributor && distributor !== "all") {
      filteredWarehouses = filteredWarehouses.filter(
        (warehouse) => warehouse.distributor.user_id === distributor
      );
    }
  ...
  async getWarehouseById(warehouseId: string): Promise<Invenstory | null> {
    await delay(200);
    return (
      warehouses.find((warehouse) => warehouse.invenstory_id === warehouseId) ||
      null
    );
  },
  ...
  async createWarehouse(
    ...
    const distributor = users.find(
      (user) => user.user_id === warehouseData.distributor_id
    );
    if (!distributor) {
    ...
    const existingWarehouse = warehouses.find(
      (warehouse) =>
        warehouse.distributor.user_id === warehouseData.distributor_id
    );
  ...
  async updateWarehouse(
    ...
    const warehouseIndex = warehouses.findIndex(
      (warehouse) => warehouse.invenstory_id === warehouseData.invenstory_id
    );
    if (warehouseIndex === -1) {
    ...
    const updatedWarehouse: Invenstory = {
      ...
      business_license:
        warehouseData.business_license ?? currentWarehouse.business_license,
      invenstory_address:
        warehouseData.invenstory_address ?? currentWarehouse.invenstory_address,
      invenstory_lat:
        warehouseData.invenstory_lat ?? currentWarehouse.invenstory_lat,
      invenstory_lng:
        warehouseData.invenstory_lng ?? currentWarehouse.invenstory_lng,
      invenstory_img:
        warehouseData.invenstory_img ?? currentWarehouse.invenstory_img,
  ...
  async deleteWarehouse(warehouseId: string): Promise<boolean> {
    ...
    const warehouseIndex = warehouses.findIndex(
      (warehouse) => warehouse.invenstory_id === warehouseId
    );
    if (warehouseIndex === -1) {
      throw new Error("Không tìm thấy kho hàng");
    }
  ...
  async toggleWarehouseLock(warehouseId: string): Promise<Invenstory> {
    ...
    warehouses[warehouseIndex].is_locked =
      !warehouses[warehouseIndex].is_locked;
...
export const diseaseAPI = {
  async getDiseases(
    ...
    if (status && status !== "all") {
      const isActive = status === "active";
      filteredDiseases = filteredDiseases.filter(
        (disease) => disease.is_active === isActive
      );
    }
    ...
    if (affected_crop && affected_crop !== "all") {
      filteredDiseases = filteredDiseases.filter((disease) =>
        disease.affected_crops.some((crop) =>
          crop.toLowerCase().includes(affected_crop.toLowerCase())
        )
      );
    }
  ...
  async createDisease(diseaseData: CreateDiseaseRequest): Promise<Disease> {
    ...
    const existingDisease = diseases.find(
      (disease) =>
        ...
        diseaseData.disease_name.toLowerCase()
    );
  ...
  async updateDisease(diseaseData: UpdateDiseaseRequest): Promise<Disease> {
    ...
    const diseaseIndex = diseases.findIndex(
      (disease) => disease.disease_id === diseaseData.disease_id
    );
    if (diseaseIndex === -1) {
    ...
    const updatedDisease: Disease = {
      ...
      scientific_name:
        diseaseData.scientific_name ?? currentDisease.scientific_name,
      affected_crops:
        diseaseData.affected_crops ?? currentDisease.affected_crops,
      symptoms: diseaseData.symptoms ?? currentDisease.symptoms,
      prevention_tips:
        diseaseData.prevention_tips ?? currentDisease.prevention_tips,
...
export const activeIngredientAPI = {
  async getActiveIngredients(
  ...
  ): Promise<PaginatedResponse<ActiveIngredient>> {
    await delay(300);
    ...
    if (status && status !== "all") {
      const isActive = status === "active";
      filteredIngredients = filteredIngredients.filter(
        (ingredient) => ingredient.is_active === isActive
      );
    }
    ...
    if (hazard_level && hazard_level !== "all") {
      filteredIngredients = filteredIngredients.filter(
        (ingredient) => ingredient.hazard_level === hazard_level
      );
    }
  ...
  async createActiveIngredient(
    ingredientData: CreateActiveIngredientRequest
    ...
    const existingIngredient = activeIngredients.find(
      (ingredient) =>
        ...
        ingredientData.ingredient_name.toLowerCase()
    );
  ...
  async updateActiveIngredient(
    ingredientData: UpdateActiveIngredientRequest
  ): Promise<ActiveIngredient> {
    await delay(400);
    ...
    const updatedIngredient: ActiveIngredient = {
      ...currentIngredient,
      ingredient_name:
        ingredientData.ingredient_name ?? currentIngredient.ingredient_name,
      description: ingredientData.description ?? currentIngredient.description,
      hazard_level:
        ingredientData.hazard_level ?? currentIngredient.hazard_level,
      chemical_formula:
        ingredientData.chemical_formula ?? currentIngredient.chemical_formula,
  ...
  async toggleActiveIngredientStatus(
    ingredientId: string
  ): Promise<ActiveIngredient> {
    await delay(200);
    ...
    activeIngredients[ingredientIndex].is_active =
      !activeIngredients[ingredientIndex].is_active;
...
export const productAPI = {
  async getProducts(
    ...
    if (distributor && distributor !== "all") {
      filteredProducts = filteredProducts.filter(
        (product) => product.distributor.user_id === distributor
      );
    }
    ...
    if (price_min !== undefined) {
      filteredProducts = filteredProducts.filter(
        (product) => product.unit_product_price >= price_min
      );
    }
    if (price_max !== undefined) {
      filteredProducts = filteredProducts.filter(
        (product) => product.unit_product_price <= price_max
      );
    }
  ...
  async createProduct(productData: CreateProductRequest): Promise<Product> {
    ...
    const distributor = users.find(
      (user) => user.user_id === productData.distributor_id
    );
    if (!distributor) {
      throw new Error("Không tìm thấy nhà phân phối");
    }
    ...
    const productCategories = categories.filter((cat) =>
      productData.category_ids.includes(cat.category_id)
    );
  ...
  async updateProduct(productData: UpdateProductRequest): Promise<Product> {
    ...
    if (productData.category_ids) {
      updatedCategories = categories.filter((cat) =>
        productData.category_ids!.includes(cat.category_id)
      );
    ...
    const updatedProduct: Product = {
      ...
      usage_instructions:
        productData.usage_instructions ?? currentProduct.usage_instructions,
      unit_product_price:
        productData.unit_product_price ?? currentProduct.unit_product_price,
  ...
  async addProductIngredient(
    ...
    const existingRelation = productIngredients.find(
      (pi) =>
        pi.product.product_id === data.product_id &&
        pi.active_ingredient.ingredient_id === data.ingredient_id
    );
    if (existingRelation) {
  ...
  async updateProductIngredient(
    ...
    const piIndex = productIngredients.findIndex(
  ...
  async removeProductIngredient(productIngredientId: string): Promise<boolean> {
    ...
    const product = products.find(
      (p) => p.product_id === removedPI.product.product_id
    );
  ...
  async addProductDisease(
    data: ProductDiseaseRequest
  ): Promise<ProductDisease> {
    await delay(300);
    ...
    const existingRelation = productDiseases.find(
      (pd) =>
        pd.product.product_id === data.product_id &&
        pd.disease.disease_id === data.disease_id
    );
    if (existingRelation) {
  ...
  async updateProductDisease(
    productDiseaseId: string,
    data: Partial<ProductDiseaseRequest>
  ): Promise<ProductDisease> {
    await delay(300);

    const pdIndex = productDiseases.findIndex(
      (pd) => pd.product_disease_id === productDiseaseId
    );
    if (pdIndex === -1) {
  ...
  async removeProductDisease(productDiseaseId: string): Promise<boolean> {
    ...
    // Update product's diseases array
    const product = products.find(
      (p) => p.product_id === removedPD.product.product_id
    );
    if (product && product.product_diseases) {
      product.product_diseases = product.product_diseases.filter(
        (pd) => pd.product_disease_id !== productDiseaseId
      );
    }
  ...
  async removeProductImage(imageId: string): Promise<boolean> {
    ...
    // Update product's images array
    const product = products.find(
      (p) => p.product_id === removedImage.product.product_id
    );
...
export const storeRequestAPI = {
  async getStoreRequests(
    filters: StoreRequestFilters = {}
  ): Promise<PaginatedResponse<StoreOwnerRequest>> {
    await delay(300);
    ...
    if (status && status !== "all") {
      if (status === "pending") {
        filteredRequests = filteredRequests.filter(
          (request) => !request.request_status && !request.approved_date
        );
  ...
  async approveStoreRequest(requestId: string): Promise<StoreOwnerRequest> {
    ...
    const requestIndex = storeRequests.findIndex(
      (req) => req.store_owner_request_id === requestId
    );
    if (requestIndex === -1) {
    ...
    const userIndex = users.findIndex(
      (user) => user.user_id === request.user.user_id
    );
    if (userIndex !== -1) {
      users[userIndex] = {
        ...users[userIndex],
