...
import { StoreRequestService } from "@/lib_dashboard/services/store-request-service";
import {
...
} from "@/lib_dashboard/store/store-request-store";
...
export const useStoreRequestActions = () => {
  ...
  const [approveModalOpen, setApproveModalOpen] = useAtom(
    approveRequestModalAtom
  );
  ...
  const rejectRequest = useCallback(async () => {
    ...
    try {
      await StoreRequestService.rejectRequest(
        selectedRequestId,
        rejectionReason
      );
      toast({
