...
import { WarehouseService } from "@/lib_dashboard/services/warehouse-service";
import {
...
} from "@/lib_dashboard/store/warehouse-store";
      ...
      try {
        ...
        toast({
          ...
          description:
            error instanceof Error
              ? error.message
              : "Không thể cập nhật trạng thái kho hàng",
          variant: "destructive",
        });
      }
    },
...
export function useWarehouseForm() {
  ...
  const [selectedWarehouseId, setSelectedWarehouseId] = useAtom(
    selectedWarehouseIdAtom
  );
  ...
  const createWarehouse = useCallback(async () => {
    try {
      ...
      toast({
        title: "Lỗi",
        description:
          error instanceof Error ? error.message : "Không thể tạo kho hàng",
        variant: "destructive",
      });
      return false;
    }
  ...
  const deleteWarehouse = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Lỗi",
        description:
          error instanceof Error ? error.message : "Không thể xóa kho hàng",
        variant: "destructive",
      });
      return false;
    }
