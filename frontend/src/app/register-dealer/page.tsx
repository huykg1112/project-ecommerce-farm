...
export default function RegisterDealerPage() {
  ...
  const checkStepCompletion = (step: number) => {
    switch (step) {
      ...
      case 2: // Thông tin cá nhân
        const { fullName, email, phone, idNumber, businessLicense } = formData.personalInfo
        setIsStepCompleted(!!fullName && !!email && !!phone && !!idNumber && !!businessLicense)
        break
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast({
        ...
        description: "Yêu cầu đăng ký đại lý của bạn đã được gửi. Chúng tôi sẽ liên hệ với bạn sớm.",
      })
    } catch (error) {
  ...
  return (
    <div className="container max-w-4xl mx-auto py-8 px-4">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-center mb-2">Đăng ký trở thành đại lý</h1>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        {currentStep === 2 && (
          <PersonalInfoStep data={formData.personalInfo} updateData={(data) => updateFormData("personalInfo", data)} />
        )}

        {currentStep === 3 && (
          <DealerInfoStep data={formData.dealerInfo} updateData={(data) => updateFormData("dealerInfo", data)} />
        )}

        {currentStep === 4 && (
          <TermsStep
            termsAccepted={formData.termsAccepted}
            updateTermsAccepted={(accepted) => updateFormData("termsAccepted", accepted)}
          />
        )}
      ...
      <div className="flex justify-between">
        ...
        {currentStep === 1 && (
          <Button onClick={handleBack} variant="outline" className="opacity-0 pointer-events-none">
            <ChevronLeft className="mr-2 h-4 w-4" /> Quay lại
          </Button>
        )}
        ...
        {currentStep === 4 && (
          <Button onClick={handleSubmit} disabled={!isStepCompleted} className="bg-green-600 hover:bg-green-700">
            Hoàn thành đăng ký
          </Button>
        )}
