        ...
        <Table>
          ...
          <TableBody>
            {requests.map((request) => (
              <TableRow
                key={request.store_owner_request_id}
              ...
              >
                ...
                <TableCell>
                  <Badge
                    variant={getStatusBadge(request).variant}
                    className={getStatusBadge(request).className}
                  >
                    {getStatusBadge(request).label}
                  </Badge>
                  {request.approved_date && (
                    <div className="text-xs text-[#74a65d] mt-1">
                      {formatDate(new Date(request.approved_date))}
                    </div>
                  )}
                </TableCell>
                <TableCell>
                  <DropdownMenu>
                    ...
                    >
                      <DropdownMenuItem
                        onClick={() =>
                          onViewDetails(request.store_owner_request_id)
                        }
                      ...
                      {isPending(request) && (
                        <>
                          <DropdownMenuItem
                            onClick={() =>
                              onApprove(request.store_owner_request_id)
                            }
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              onReject(request.store_owner_request_id)
                            }
