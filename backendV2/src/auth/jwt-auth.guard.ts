...
export class JwtAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Kiểm tra metadata với key từ hằng số IS_PUBLIC_KEY
    ...
    try {
      ...
      const [user, storedToken] = await Promise.all([
        this.userService.findUserById(decoded.sub),
      ...
      if (
        ...
        (storedToken.access_token_expires_at &&
          storedToken.access_token_expires_at < new Date())
      ) {
        throw new UnauthorizedException(
