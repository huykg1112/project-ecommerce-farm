...
import { Eye, EyeOff, TrendingUp, Users, PieChartIcon } from "lucide-react"
import type { RegistrationData, RoleDistribution, ChartVisibility } from "@/lib/store/user-statistics-store"
...
export const StatisticsCharts = memo<StatisticsChartsProps>(
  ({ registrationTrends, roleDistribution, chartVisibility, onToggleVisibility, timeRange }) => {
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className={`border-[#90c577] text-[#44703d] hover:bg-[#accc8b]/20 ${
                    chartVisibility.customers ? "bg-[#90c577]/20" : "bg-transparent"
                  }`}
                >
                  {chartVisibility.customers ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                ...
                <Button
                  ...
                  className={`border-[#90c577] text-[#44703d] hover:bg-[#accc8b]/20 ${
                    chartVisibility.distributors ? "bg-[#74a65d]/20" : "bg-transparent"
                  }`}
                >
                  {chartVisibility.distributors ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                  <span className="ml-1">Đại lý</span>
                </Button>
              </div>
            </div>
            <p className="text-sm text-[#74a65d]">Số lượng người dùng đăng ký mới theo thời gian</p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={filteredTrendsData}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-[#accc8b]/50" />
                <XAxis dataKey="period" className="text-[#74a65d]" fontSize={12} tick={{ fill: "#74a65d" }} />
                <YAxis className="text-[#74a65d]" fontSize={12} tick={{ fill: "#74a65d" }} />
                <Tooltip contentStyle={tooltipStyle} />
                <Legend />
                {chartVisibility.customers && (
                  <Bar dataKey="customers" stackId="a" fill="#90c577" name="Khách hàng" radius={[0, 0, 0, 0]} />
                )}
                {chartVisibility.distributors && (
                  <Bar
                    ...
                    fill="#74a65d"
                    name="Đại lý"
                    radius={chartVisibility.customers ? [4, 4, 0, 0] : [4, 4, 4, 4]}
                  />
                )}
              </BarChart>
            </ResponsiveContainer>
        ...
        <Card className="card-agricultural">
          <CardHeader>
            ...
            <p className="text-sm text-[#74a65d]">Tỷ lệ phân bố người dùng theo vai trò</p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
            ...
            <div className="mt-4 space-y-2">
              {roleDistribution.map((item, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="text-[#44703d] border-[#90c577]">
        ...
        <Card className="card-agricultural lg:col-span-3">
          <CardHeader>
            ...
            <p className="text-sm text-[#74a65d]">Biểu đồ đường thể hiện xu hướng đăng ký theo thời gian</p>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={registrationTrends}>
                ...
                <Line
                  ...
                  activeDot={{ r: 7, stroke: "#599146", strokeWidth: 2, fill: "#90c577" }}
                  name="Tổng đăng ký"
                />
                {chartVisibility.customers && (
                  <Line
