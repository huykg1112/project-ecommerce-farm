...
import { Users, Building2, Package, TrendingUp, AlertTriangle } from "lucide-react"
...
export const SummaryCards = memo<SummaryCardsProps>(
  ({ newUsers, newDistributors, productsSold, totalRevenue, timeRange, userRole }) => {
    const cards = useMemo(() => {
      const baseCards = [
        {
        ...
        {cards.map((card, index) => (
          ...
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-semibold text-agricultural-secondary">{card.title}</CardTitle>
              <div className={`p-2 rounded-lg ${card.bgColor}`}>
                <card.icon className={`h-5 w-5 ${card.color}`} />
              </div>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-agricultural-primary mb-1">{card.value}</div>
              <p className="text-xs text-agricultural-secondary">
                <span className="text-primary-strong font-medium">{card.change}</span> so với kỳ trước
              </p>
            </CardContent>
          </Card>
        ))}
