...
import { Users, Building2, Package, Clock } from "lucide-react"
  ...
  return (
    <Card className="card-agricultural">
      <CardHeader>
        ...
        <p className="text-sm text-agricultural-secondary">Các chỉ số quan trọng của hệ thống</p>
      </CardHeader>
      <CardContent>
          ...
          {quickStats.map((stat, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-agricultural-primary">{stat.label}</span>
              </div>
              <span className="text-lg font-bold text-agricultural-primary">{formatNumber(stat.value)}</span>
            </div>
          ))}
        </div>
      </CardContent>
